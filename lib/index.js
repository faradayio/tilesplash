...
var pgMiddleware = function(dbOptions){
  return function(req, res, next){
    ...
    req.db.query = function(sql, bindvars, callback){
      ...
      var connOpts = dbOptions[Math.floor(Math.random()*dbOptions.length)];
...
var Tilesplash = function(dbOptions, cacheType, cacheOptions, instrument){
  ...
  var dbCacheKey = []
  dbOptions = dbOptions.map(function (opts) {
    if (typeof opts === 'string') {
      dbCacheKey.push(opts);
      return opts;
    } else if (typeof opts === 'object') {
      var newOpts = { poolSize: 64 };
      var optsKey = {};
      Object.keys(opts).forEach(function (key) {
        newOpts[key] = opts[key];
        if (key !== 'password') {
          optsKey[key] = opts[key];
        }
        dbCacheKey.push(optsKey);
      });
      return newOpts;
    } else {
  ...
  this.dbCacheKey = JSON.stringify(dbCacheKey);
